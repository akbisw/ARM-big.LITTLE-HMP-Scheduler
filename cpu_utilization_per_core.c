/* Read CPU Utilization per core, Voltage+Current+Power per Cluster Node and Temperature per big core (Temp readings only available on the big cores).
 * Store the scraped data in a text format or time series database.
 * Shows per core CPU utilization of the system using the source below:
 * Credit: http://phoxis.org/2013/09/05/finding-overall-and-per-core-cpu-utilization
 */
#include <stdio.h>
#include <unistd.h>
...
int main (void)
{
  // unsigned long long int is atleast as wide as long int. Usually denotes 64 bit int.
  FILE *fp;
